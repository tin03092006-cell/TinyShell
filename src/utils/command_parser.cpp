...
std::vector<std::string> parse_command(const std::string& input) {
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    if (c == '\\' && i + 1 < input.size()) {
      ...
      has_token = true;
               ...
               (isspace(static_cast<unsigned char>(c)) || c == '&')) {
      if (has_token || !arg.empty()) {
        args.push_back(arg);
        arg.clear();
        has_token = false;
      }
  ...
  if (in_quotes) {
    throw std::invalid_argument("Error: Unmatched quote in command.");
  }
...
// Hàm nối các tham số thành một chuỗi dòng lệnh hoàn chỉnh (có xử lý ngoặc kép
// và escape)
